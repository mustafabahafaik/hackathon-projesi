...
- **`apps/api/src/services/escrow-orchestrator/soroswap.ts`, hosted Soroswap API yerine router kontratına doğrudan çağrı:** Mimari doküman "Soroswap Router API" (`api.soroswap.finance`, hosted aggregator) diyor; bu servis onun yerine gerçek testnet router **kontratına** (`SOROSWAP_ROUTER_ADDRESS`) doğrudan cross-contract call yapıyor. Neden: hosted API `sk_...` anahtarı istiyor, `POST /register` "hesap bir yönetici tarafından aktive edilene kadar devre dışı kalır" diyor — bu oturumun kontrolünde/görüş alanında olmayan, süresi belirsiz bir insan-onayı adımı (boş denendi: anahtarsız her swap endpoint'i `403 Forbidden` döndürdü, ampirik olarak doğrulandı). Router kontratının kendisi bu anahtara ihtiyaç duymuyor — hosted API zaten bu kontrata çağrı yapıyor, dolayısıyla doğrudan çağırmak da gerçek Soroswap protokolünü testnet'te çağırmak, mock değil. Sonuç: otomatik çoklu-hop rota bulma yok (bu, hosted Aggregator'ın işi) — `path` parametresi elle verilmeli, varsayılan doğrudan iki-hop `[assetIn, assetOut]`. Ayrıntı için `soroswap.ts`'in modül yorumuna bak; gerçek testnet kanıtı için `apps/api/src/scripts/soroswap-integration-test.ts`.
